...
data['waktu'] = data['waktu'].map(
    {'Pagi': 0, 'Siang': 1, 'Malam': 2, 'Dini Hari': 3})

# Pemrosesan kolom wilayah
data['wilayah'] = data['wilayah'].map({'Jakarta Selatan': 0, 'Jakarta Barat': 1,
                                      'Jakarta Utara': 2, 'Jakarta Timur': 3, 'Jakarta Pusat': 4, 'Kepulauan Seribu': 5})
...
X = data[['waktu', 'wilayah', 'kelembaban_min',
          'kelembaban_max', 'suhu_min', 'suhu_max']]
y = data['cuaca']
...
X_train_scaled = pd.DataFrame(scaler.fit_transform(
    X_train_imputed), columns=X_train_imputed.columns)
X_test_scaled = pd.DataFrame(scaler.transform(
    X_test_imputed), columns=X_test_imputed.columns)
...
def cbr_knn():
    ...
    try:
        ...
        result_label.configure(
            text="Input harus berupa angka untuk kelembaban dan suhu.")
        return
    ...
    new_data_scaled = pd.DataFrame(scaler.transform(
        new_data_imputed), columns=new_data_imputed.columns)
    ...
    result_label.configure(text="Prediksi Cuaca: " +
                           predicted_weather_label[0])
    accuracy_label.configure(
        text="Akurasi Model: " + str(round(accuracy_score(y_test, y_pred) * 100, 2)) + "%")
    precision_label.configure(
        text="Presisi: " + str(round(precision, 2)) + "%")
...
waktu_label = ttk.Label(
    input_frame, text="Waktu (Pagi, Siang, Malam, Dini Hari):", style='TLabel')
waktu_label.grid(row=0, column=0, sticky='w')
waktu_entry = ttk.Entry(input_frame)
waktu_entry.grid(row=0, column=1)

wilayah_label = ttk.Label(
    input_frame, text="Wilayah (Jakarta Selatan, Jakarta Barat, Jakarta Utara, Jakarta Timur, Jakarta Pusat, Kepulauan Seribu):", style='TLabel')
wilayah_label.grid(row=1, column=0, sticky='w')
wilayah_entry = ttk.Entry(input_frame)
wilayah_entry.grid(row=1, column=1)

kelembaban_min_label = ttk.Label(
    input_frame, text="Kelembaban Minimum:", style='TLabel')
...
kelembaban_max_label = ttk.Label(
    input_frame, text="Kelembaban Maksimum:", style='TLabel')
...
predict_button = ttk.Button(
    button_frame, text="Prediksi CBR", command=cbr_knn, style='TButton')
predict_button.pack(side='left', padx=5)

show_distribution_button = ttk.Button(
    button_frame, text="Tampilkan Distribusi Cuaca", command=show_weather_distribution, style='TButton')
